...
namespace BoidManager
{
	...
	const float GROUP_DISTANCE = 1.0f;
	...
	void Update(float deltaTime)
	{
		for (size_t i = 0; i < BOID_AMOUNT; i++)
		{
			...
			m_Boids[i].Position += m_Boids[i].Velocity * MOVE_SPEED;
	...
	glm::vec3 GetCohesionVelocity(int index)
	{
		...
		for (size_t i = 0; i < BOID_AMOUNT; i++)
		{
			...
			if (glm::length(distance) > GROUP_DISTANCE)
			{
				continue;
			}
		...
		glm::vec3 resultVelocity = (pseudoCenter - m_Boids[index].Position) * 0.01f;
	...
	glm::vec3 GetAlignmentVelocity(int index)
	{
		...
		glm::vec3 resultVelocity = (pseudoVelocityAvg - m_Boids[index].Velocity) * 0.125f;
